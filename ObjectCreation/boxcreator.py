...
def notch_line(length, notchWidth, materialWidth, inside = False, drawNotches = True):
    if drawNotches == False:
        if inside:
            len2 = length - 2 * materialWidth
        else:
            len2 = length

        points=[FreeCAD.Vector(0.0, 0.0, 0.0), FreeCAD.Vector(len2, 0.0, 0.0)]
...
def notch_holes(length, notchWidth, materialWidth, overhang, drawHoles = True, overhangLeft = 0, overhangRight = 0):
    ...
    points = [FreeCAD.Vector(-overhangLeft, 0, 0), FreeCAD.Vector(-overhangLeft, -overhang, 0), FreeCAD.Vector(length + overhangRight, -overhang, 0), FreeCAD.Vector(length + overhangRight, 0, 0)]
